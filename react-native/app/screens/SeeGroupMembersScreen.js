...
export default function HomeScreen({navigation, route}) {
  ...
  const searchMembers = (cb) =>{
    axios.get(window.api_prefix+"/getGroupMembers?nameSearch="+nameSearch+"&groupId="+route.params.groupId).then(resp=>{
  ...
  const removeFromGroup = (userArrayKey) =>{
    const userId = searchResults[userArrayKey].user_id;
    const userName = searchResults[userArrayKey].name;

    axios.post(window.api_prefix+"/removeMemberFromGroup", {userId:userId, userName:userName, groupId: route.params.groupId}).then(resp=>{
      ...
      return (
        <View key={key} style={{paddingHorizontal:10, paddingVertical:15, borderBottomWidth:1, borderBottomColor:"#e9e9e9", flexDirection:"row", alignItems:"center"}}>
          ...
          <Text style={[globalStyle.regularText, {flex:1}]}>{value.name}</Text>
          {
              value.name != username
              ?
              <TouchableWithoutFeedback onPress={()=>{removeFromGroup(key)}}>
                    ...
                    <Text style={[{color:"white"}, globalStyle.regularTextBold]}>{"Remove"}</Text>
                    </View>
        ...
        <SemiFullPageScreen>
          <View style={{height:windowHeight, width:windowWidth}}>
            <View style={{flex:1}} style={{backgroundColor:"#f4f4f4", paddingHorizontal:15, borderBottomColor:"#e9e9e9", borderBottomWidth:1, paddingBottom:15, flexDirection:"row", alignItems:"center", justifyContent:"center"}}>
              ...
              <Text style={[globalStyle.title1, {color:"rgba(0,0,0,0.9)", alignSelf:"stretch", flex:1, textAlign:"center"}]}>See members</Text>
            ...
            <View>
              <View style={{flexDirection:"row", alignItems:"center", paddingLeft:10}}>
                ...
                <TextInput value={nameSearch} onChangeText={(value)=>{handleSearchTyping(value)}} style={[styles.textInput, globalStyle.regularText]} placeholder="Search group members"/>
              </View>
              <View style={{backgroundColor:"#f4f4f4", paddingHorizontal:10, paddingVertical:10, borderTopColor:"#e9e9e9", borderBottomColor:"#e9e9e9", borderTopWidth:1, borderBottomWidth:1}}>
                <Text style={[globalStyle.regularTextBold]}>{nameSearch.length>0? "Search results":"Recommended members"}</Text>
              </View>
            </View>
            <View style={{flex:1}}>
