...
app.post("/uploadChallengePicture", sessionMiddlewares.sessionLoggedInCheck, (request, response) => {
    const form = new multiparty.Form();
    form.parse(request, async (error, fields, files) => {
        if (error) {
            console.log(err);
            return response.status(200).send({ message: "There has been a server error, please try again.", success:false });
        };
        try {
            console.log(fields);
            ...
            if(fileStats.size > 25000000){
                //25mb
                return response.status(200).send({ message: "Your image size is too big. Please try uploading an image under 25mb.", success:false });
            }
            ...
            if (type.mime == "image/png" || type.mime == "image/jpg" || type.mime == "image/jpeg" || type.mime == "image/svg") {
                ...
                pool.query("INSERT INTO Picture (url, user_id, challenge_start_timestamp, challenge_group_id, timestamp) VALUES (?,?,?,?,?)", [data.Location, response.locals.userId, fields.startTimestamp[0], fields.groupId[0], nowTimestamp], (errSql, resultSql)=>{
                    if(errSql){
                        return response.status(200).send({ message: "There has been a server error, please try again.", success:false });
                    }
                ...
                return response.status(200).send({ message: "Image file format only.", success:false });
            }
        } catch (err) {
            console.log(err);
            return response.status(200).send({ message: "There has been a server error, please try again.", success:false });
