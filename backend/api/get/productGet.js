...
app.get("/searchHistory", async (req, res)=>{
    try{
        ...
        res.status(200).send({communitiesHistory, groupsHistory});
    }catch(e){
...
app.get("/trendingTopics", async(req, res)=>{
    ...
    try{
        res.status(200).send({communities:communities, groups:groups});
    }catch(err){
        res.status(200).send({message:"There has been a server error, please try again.", success:false});
    }
});
...
app.get("/groupSignals", async(req, res)=>{
    let posts = [
        {
            id:1,
            groupid:1,
